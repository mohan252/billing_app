...
namespace Dundas.Charting.Utilities
{
	...
	public class PieCollectedDataHelper
	{
		...
		public PieCollectedDataHelper(Chart chartControl)
		{
			...
			this.chartControl.PostPaint += new Dundas.Charting.WinControl.PaintEventHandler(this.chart_PostPaint);
		}
		...
		public void ShowSmallSegmentsAsSupplementalPie(string seriesName)
		{
			...
			if(this.series.Type != SeriesChartType.Pie &&
				this.series.Type != SeriesChartType.Doughnut)
			{
			...
			if( CreateCollectedPie() )
			{
                ...
                this.chartControl.Legends[0].Docking = LegendDocking.Bottom;
				...
				this.supplementalSeries.Type = this.series.Type;
		...
		private bool CreateCollectedPie()
		{
			...
			if(!ShowCollectedDataAsOneSlice)
			{
				foreach(DataPoint dataPoint in this.supplementalSeries.Points)
				{
					...
					dataPointCollected.ShowLabelAsValue = false;
				}
			}
			...
			if(collectedValue > 0.0)
			{
				...
				if(this.ShowCollectedDataAsOneSlice)
				{
					...
					colectedDataPoint.ShowInLegend = false;
                ...
                supplementalSeries.Label = "#PERCENT";
                return true;
			}
		...
		private void chart_PostPaint(object sender, Dundas.Charting.WinControl.ChartPaintEventArgs e)
		{
			if(sender is ChartArea)
			{
					...
					area.Name == this.supplementalChartArea.Name)
				{
					...
					{
						e.ChartGraphics.DrawLine(pen, p1, p3);
						e.ChartGraphics.DrawLine(pen, p2, p4);
					}
				}
			}
		}
